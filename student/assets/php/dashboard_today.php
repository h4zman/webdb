...
$student_id = "191991"; // amir - ade student id 2, schdule id 412,413
...
$student_id = $student["student_id"]; // 1 je data dpt
...
//echo '<pre>'; print_r($schedule_id); echo '</pre>'; // print out the data
...
if ($length <2)
 {
        ...
        $sql = "SELECT * FROM `schedule_class` WHERE `Schedule_id`='$schedule_id' AND `Day`= '$today' ;";
      ...
      //  echo '<pre>'; print_r($course_info); echo '</pre>'; // print out the data
        ...
        $new_array[0] = array_merge($array[0],$course_info);
      ...
      //  echo '<pre>'; print_r($lecturer_info); echo '</pre>'; // print out the data
        ...
        $new_array[0] =array_merge($new_array[0],$lecturer_info);
...
else if($length >1)
{
    ...
    for($i=0;$i<$length;$i++)
    {
       ...
       // echo $schedule_id_new;
        $sql = "SELECT * FROM `schedule_class` WHERE `Schedule_id`='$schedule_id_new' AND `Day`= '$today';";
        $classday=$conn->query($sql) or die($conn->error);
